        ...
        await supabase
          ...
          .then(async (res) => {
            ...
            if (data && statusText === 'OK') {
              ...
              const lo1_media_name = `${title}_lo1_media.${lo1_media?.type.split("/").pop()}`;

              const lo2_media = req?.files?.lo2_media;
              const lo2_mediaFile = fs.readFileSync(lo2_media.path);
              const lo2_media_name = `${title}_lo2_media.${lo2_media?.type.split("/").pop()}`;

              const lo3_media = req?.files?.lo3_media;
              const lo3_mediaFile = fs.readFileSync(lo3_media.path);
              const lo3_media_name = `${title}_lo3_media.${lo3_media?.type.split("/").pop()}`;
